	...
	var MiniInput = (function () {
		...
		MiniInput.wxinputFocus = function (e) {
			...
			MiniAdpter.window.wx.showKeyboard({
				defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: 'done', success: function (res) {
				}, fail: function (res) {
				}
			});
	...
	var MiniFileMgr = (function (_super) {
		...
		MiniFileMgr.exits = function (fileName, callBack) {
			...
			MiniFileMgr.fs.getFileInfo({
				filePath: nativeFileName, success: function (data) {
		...
		MiniFileMgr.readNativeFile = function (filePath, callBack) {
			MiniFileMgr.fs.readFile({
				filePath: filePath, encoding: "", success: function (data) {
		...
		MiniFileMgr.down = function (fileUrl, encoding, callBack, readyUrl) {
			...
			var downloadTask = MiniFileMgr.wxdown({
				url: fileUrl, filePath: savePath, success: function (data) {
		...
		MiniFileMgr.copyFile = function (tempFilePath, readyUrl, callBack) {
			...
			MiniFileMgr.fs.copyFile({
				srcPath: tempFilePath, destPath: saveFilePath, success: function (data) {
					if (!fileObj) {
						MiniFileMgr.onSaveFile(readyUrl, tempFileName);
						callBack != null && callBack.runWith([0]);
					} else {
		...
		MiniFileMgr.onSaveFile = function (readyUrl, md5Name) {
			...
			MiniFileMgr.fs.writeFile({
				filePath: MiniFileMgr.fileNativeDir + "/" + MiniFileMgr.fileListName, encoding: 'utf8', data: JSON.stringify(MiniFileMgr.filesListObj), success: function (data) {
				}, fail: function (data) {
				}
			});
		}
		...
		*/
		__proto.load = function (url) {
			...
			function _clearSound() {
				_$this._sound.onCanplay(function () { });
				_$this._sound.onError(function () { });
			}
		}
